...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.markdown(f"<p class='rcount'>Showing {len(filtered)} pop-up{'s' if len(filtered)!=1 else ''}</p>", unsafe_allow_html=True)
    ...
    for dist_key in ["seongsu","hannam","hongdae","gangnam","others"]:
        sub_df = filtered[filtered["d"] == dist_key]
        if sub_df.empty:
            continue
        ...
        for i, (_, row) in enumerate(sub_df.iterrows()):
            ...
            card_html = f"""
<div class="pcard">
  <div class="pcard-bar" style="background:{bar_color}"></div>
  <div class="pcard-inner">
    <div class="pcard-top">
      <span class="pcard-cat {cat_css}">{row["cat"]}</span>
      <div class="pcard-right">
        <span class="pcard-yr {yr_css}">{row["yr"]}</span>
        {hot_html}
      </div>
    </div>
    <div class="pcard-name">{row["name"]}</div>
    <div class="pcard-brand">{row["brand"]}</div>
    <div class="pcard-desc">{row["desc"]}</div>
    ...
    <div class="pcard-why">📝 {row["why"]}</div>
  </div>
</div>"""
