    ...
    if st.session_state.get("show_img"):
        ...
        <div style="background:#0d1221;border:1px solid rgba(255,107,157,0.3);
          ...
          <div style="background:linear-gradient(90deg,rgba(255,107,157,0.08),transparent);
            ...
            <div style="font-family:'DM Mono',monospace;font-size:11px;color:#6b7280;margin-top:3px">
              Search: {img_query}
            </div>
          </div>
        </div>
        ...
        # ── Fetch real images via DuckDuckGo (no API key needed) ────────────
        def fetch_ddg_images(query: str, max_count: int = 6) -> list:
            """Scrape DuckDuckGo image search for real popup photos."""
            ...
            try:
                ...
                api_url = (
                    ...
                    f"q={urllib.parse.quote(query)}&o=json&p=1"
                    f"&s=0&u=bing&f=,,,,,&l=us-en&vqd={vqd}"
                )
                ...
                for item in results[:max_count]:
                    ...
                    if url and url.startswith("http"):
                        urls.append({"url": url, "title": item.get("title",""), "source": item.get("url","")})
                return urls
            except Exception as e:
                return []

        with st.spinner("🔍 Searching for pop-up photos..."):
            # Try specific query first, fall back to broader Seoul popup search
            imgs = fetch_ddg_images(img_query, 6)
            if len(imgs) < 2:
                imgs = fetch_ddg_images(f"Seoul popup store {img_name}", 6)
            if len(imgs) < 2:
                imgs = fetch_ddg_images("Seoul Seongsu popup store 2025 2026", 6)

        if imgs:
            img_cols = st.columns(3)
            for idx, img_data in enumerate(imgs[:6]):
                with img_cols[idx % 3]:
                    try:
                        st.image(
                            img_data["url"],
                            caption=img_data["title"][:60] if img_data["title"] else img_name,
                            use_container_width=True,
                        )
                    except Exception:
                        st.markdown(f"""
            ...
            # Fallback: show styled placeholder with search link
            st.markdown(f"""
            <div style="background:#0d1221;border:1px solid rgba(255,255,255,0.07);
              ...
              <div style="font-family:'Syne',sans-serif;font-size:16px;font-weight:700;
                          color:#f0eee8;margin-bottom:8px">Photos not available in this environment</div>
              <div style="font-family:'DM Mono',monospace;font-size:11px;color:#6b7280;
                          line-height:1.8;max-width:400px;margin:0 auto">
                Search manually for photos:<br>
                <span style="color:#ff6b9d">"{img_query}"</span>
              </div>
              <div style="margin-top:16px">
                <a href="https://www.google.com/search?q={img_query.replace(" ", "+")}&tbm=isch"
                   target="_blank"
                   style="display:inline-block;padding:8px 20px;background:rgba(255,107,157,0.15);
