    ...
    if len(filtered_df) > 1:
        ...
        for cat in filtered_df["category"].unique():
            ...
            fig_scatter.add_trace(go.Scatter(
                x=sub["open_days"], y=sub["visitors"],
                mode="markers",
                name=cat,
                marker=dict(
                    color=clr,
                    size=sub["instagram_tags"] / sub["instagram_tags"].max() * 30 + 8,
                    opacity=0.8,
                    line=dict(width=1, color="rgba(255,255,255,0.3)"),
                ),
                text=sub["name"],
                customdata=sub[["district","satisfaction","instagram_tags"]].values,
                hovertemplate=(
                    "<b>%{text}</b><br>"
                    ...
                    "Insta Tags: %{customdata[2]:,}<extra></extra>"
                ),
            ))
        # Overall trendline via numpy (no statsmodels needed)
        x_all = filtered_df["open_days"].values.astype(float)
        y_all = filtered_df["visitors"].values.astype(float)
        if len(x_all) >= 2:
        ...
        apply_theme(fig_scatter, height=370)
        fig_scatter.update_layout(
            margin=dict(l=10, r=10, t=10, b=10),
            legend=dict(bgcolor="rgba(0,0,0,0)"),
            xaxis_title="Operating Days",
            yaxis_title="Total Visitors",
        )
        st.plotly_chart(fig_scatter, use_container_width=True)
    else:
        st.info("Not enough data points for this chart — adjust filters.")
