...
with tab1:
    ...
    if st.session_state.get("show_img"):
        st.markdown(f"""
        <div style="background:#0d1221;border:1px solid rgba(255,255,255,0.1);
          ...
          <div style="padding:16px 20px;border-bottom:1px solid rgba(255,255,255,0.07);
            ...
            <div>
              <div style="font-family:'DM Mono',monospace;font-size:9px;letter-spacing:.18em;
                          text-transform:uppercase;color:#ff6b9d;margin-bottom:4px">📸 Pop-up Photos</div>
              <div style="font-family:'Syne',sans-serif;font-size:16px;font-weight:800;color:#f0eee8">
                {st.session_state.img_name}
              </div>
            </div>
          </div>
        ...
        if st.button("✕ Close Photos", key="close_img_modal"):
            st.session_state.show_img = False
            st.rerun()

        # Fetch images via Unsplash source (no API key needed)
        query = st.session_state.img_query
        img_cols = st.columns(3)
        search_terms = [
            f"https://source.unsplash.com/400x300/?{query.replace(' ',',')}",
            f"https://source.unsplash.com/400x300/?Seoul,popup,store,{row['cat'].replace(' ','').replace('·','')}",
            f"https://source.unsplash.com/400x300/?Seoul,Korea,fashion,street",
        ]
        captions = [
            f"📍 {st.session_state.img_name}",
            "Seoul pop-up culture",
            "Seoul street scene",
        ]
        for idx, (img_url, cap) in enumerate(zip(search_terms, captions)):
            with img_cols[idx]:
